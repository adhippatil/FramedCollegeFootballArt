<?php
//-----------------------------------------
// Original Code: Algirdas Varnagiris
//-----------------------------------------
require_once("fedex_parser.php");
...
class Fedex {
    ...
    function setListRate($list) {
        $this->listRate = ($list) ? 'true' : 'false';
    }

	function setDimUnit($dimUnit) {
        $this->dimUnit = ($dimUnit) ? $dimUnit : 'IN';
    }
	...
	function setMaxWeight($weight) {
        $this->maxWeight = ($weight) ? $weight : 150;
    }

    function setDebug($debug, $email = 'nobody@nowhere.com') {
    ...
    function getPrice() {
        ...
        if ($this->listRate == 'true') {
        $str .= '        <ListRate>'.$this->listRate.'</ListRate>';
		}
        $str .= '        <OriginAddress>';
		...
		$str .= '        <Units>'.$this->dimUnit.'</Units>';
		$str .= '        </Dimensions>';
        //$str .= '        <PackageCount>'.ceil($this->bcdiv(number_format($this->weight, 1, '.', ''), '150', 3)).'</PackageCount>';
		$str .= '        <PackageCount>'.ceil($this->bcdiv(number_format($this->weight, 1, '.', ''), $this->maxWeight, 3)).'</PackageCount>';
        $str .= '    </FDXRateRequest>';
        //print($str);
        ...
        if ($this->debug) {
	        if (is_writable(getcwd())) {
				file_put_contents('fedex_debug.txt', "XML SENT TO FEDEX: \r\n\r\n $str" . "\r\n\r\n-----\r\n\r\n" . "XML RCV'D FROM FEDEX: \r\n\r\n $data");
			} else {
				mail($this->email, 'fedex_debug', "XML SENT TO FEDEX: \r\n\r\n $str" . "\r\n\r\n-----\r\n\r\n" . "XML RCV'D FROM FEDEX: \r\n\r\n $data");
			}
		}
        ...
        if (curl_errno($ch)) {
            ...
            if(@count($array['FDXRATEREPLY'][0]['ERROR'])) { // If it is error
				...
				$price->currencycode = $array['FDXRATEREPLY'][0]['ESTIMATEDCHARGES'][0]['CURRENCYCODE'][0]['VALUE'];
